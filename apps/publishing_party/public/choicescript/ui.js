...
function showMenu() {
  ...
  function menu() {
    ...
    options.push(
      ...
      {name:"Report a bug or story correction.", group:"choice", reportIssue:true},
      {name:"Download story file.", group:"choice", download:true}
    );
    options.push(
    ...
    printOptions([""], options, function(option) {
      ...
      } else if (option.reportBug || option.reportIssue) {
        reportBug();
      } else if (option.download) {
        var text = window.uploadedFiles && window.uploadedFiles['startup.txt'];
        if (text) {
          var blob = new Blob([text], {type: 'text/plain'});
          var url = URL.createObjectURL(blob);
          var a = document.createElement('a');
          a.href = url;
          a.download = (document.title || 'story') + '.txt';
          a.click();
...
function reportBug() {
  clearScreen(function() {
    var target = document.getElementById('text');
    println("Please describe the problem or story correction. Include what you expected and what actually happened.");
    println("");
    ...
    textarea.setAttribute("style", "font-size: 16px; width: 90%;");
    form.appendChild(textarea);
    target.appendChild(form);
    println("", form);
    println("", form);

    printButton("Submit", form, true);
    printButton("Cancel", target, false, function() {
      clearScreen(function() {
        setButtonTitles();
        loadAndRestoreGame();
    ...
    form.onsubmit = function(e) {
      preventDefault(e);
      var description = trim(textarea.value);
      if (!description) {
        asyncAlert("Please describe the problem before submitting.");
        return false;
      }
      var gameName = window.storeName || document.title || "(unknown)";
      var issueTitle = "Bug/correction: " + gameName;
      var issueBody = "**Game:** " + gameName + "\n";
      if (window.stats && window.stats.scene) {
        issueBody += "**Scene:** " + window.stats.scene.name + "\n";
